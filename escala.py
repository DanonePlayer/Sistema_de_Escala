...
class Tela:
    ...
    def escalas(self, event):
        ...
        self.tvw_escala = ttk.Treeview(self.frame_tvw_usuario, columns=('id', 'nome escala', 'dias da escala', 'feriados', 'finais de semana'),
                                        show='headings')
        self.tvw_escala.column('id', width=40)
        self.tvw_escala.column('nome escala', width=250)
        self.tvw_escala.column('dias da escala', width=150)
        self.tvw_escala.column('feriados', width=100)
        self.tvw_escala.column('finais de semana', width=100)
        ...
        self.tvw_escala.heading('feriados', text='Feriados')
        self.tvw_escala.heading('finais de semana', text='Finais de semana')
            ...
            # dados nao alterados
                ...
                for i in valores:
                    if nome_escala == i:
                        ...
                        break
                if not confirmar:
                    query = f'INSERT INTO escala ("nome_escala", "dias_escala", "feriados", "finais_semana") VALUES ("{nome_escala}", {dias}, {feriados}, {finais_semana});'
                    bd.inserir(query)
                    self.atualizar_tvw_escala()
    ...
    def editar_escala(self, event):
        ...
        if selecionado != ():
            ...
            self.edit_escala.title("Criar Escala")
            ...
            self.entry_dias.place(x=10, y=90)
            self.entry_dias.insert(0, lista[2])
            ...
            self.rbtn_cont_final_semanas = tk.Radiobutton(self.edit_escala, text="Sim",
                                                          variable=self.int_var_sem, value=1)
            self.rbtn_cont_final_semanas.place(x=10, y=150)
            ...
            if lista[3] == "1":
                self.int_var_sem.set(1)
            else:
                self.int_var_sem.set(2)

            self.lbl_cont_ferias = tk.Label(self.edit_escala, text="Contará os feriados?")
            self.lbl_cont_ferias.place(x=10, y=190)

            self.rbtn_cont_feriass = tk.Radiobutton(self.edit_escala, text="Sim", variable=self.int_var_fer,
                                                    value=1)
            self.rbtn_cont_feriass.place(x=10, y=210)
            ...
            if lista[4] == "1":
                self.int_var_fer.set(1)
            else:
                self.int_var_fer.set(2)
        ...
        # dados nao alterados
            ...
            for i in valores:
                if nome_escala == i and i!=lista[1]:
                    ...
                    break
            if not confirmar:
                query = f'UPDATE escala SET nome_escala="{nome_escala}", dias_escala={dias}, feriados={feriados}, finais_semana={finais_semana} WHERE escala_id={lista[0]};'
                bd.atualizar(query)
